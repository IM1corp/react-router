...
function getTurboStreamSingleFetchDataStrategy(getRouter, manifest, routeModules, ssr, deserializeError) {
	...
	}, (args, targetRoutes) => fetchAndDecodeViaTurboStream(args, targetRoutes, deserializeError), ssr);
...
async function fetchAndDecodeViaTurboStream(args, targetRoutes, deserializeError) {
	let { request } = args;
	...
	try {
		let decoded = await decodeViaTurboStream(res.body, window, deserializeError);
		let data;
		if (request.method === "GET") {
...
function decodeViaTurboStream(body, global, deserializeError) {
	return decode(body, { plugins: [(type, ...rest) => {
		if (type === "CustomError") {
			let payload = rest[0];
			let deserialized = deserializeError ? deserializeError(payload) : void 0;
			return { value: deserialized !== void 0 ? deserialized : payload };
		}
