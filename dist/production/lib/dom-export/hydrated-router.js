...
function createHydratedRouter({ getContext, instrumentations, deserializeError }) {
	initSsrInfo();
	...
	if (!ssrInfo.stateDecodingPromise) {
		...
		ssrInfo.stateDecodingPromise = UNSAFE_decodeViaTurboStream(stream, window, deserializeError).then((value) => {
	...
	let router = UNSAFE_createRouter({
		...
		instrumentations,
		dataStrategy: UNSAFE_getTurboStreamSingleFetchDataStrategy(() => router, ssrInfo.manifest, ssrInfo.routeModules, ssrInfo.context.ssr, deserializeError),
