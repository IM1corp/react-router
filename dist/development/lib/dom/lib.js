...
function createBrowserRouter(routes, opts) {
	return createRouter({
		...
		hydrationData: opts?.hydrationData || parseHydrationData(opts?.deserializeError),
		routes,
...
function parseHydrationData(deserializeError) {
...
function deserializeErrors(errors, deserializeError) {
	if (!errors) return null;
	let entries = Object.entries(errors);
	let serialized = {};
	for (let [key, val] of entries) if (val && val.__type === "CustomError") {
		let deserialized = deserializeError ? deserializeError(val.data) : void 0;
		serialized[key] = deserialized !== void 0 ? deserialized : val.data;
	} else if (val && val.__type === "RouteErrorResponse") serialized[key] = new ErrorResponseImpl(val.status, val.statusText, val.data, val.internal === true);
