...
async function singleFetchAction(build, serverMode, staticHandler, request, loadContext, handleError) {
	...
	function staticContextToResponse(context) {
		...
		if (context.errors) {
			...
			if (!build.entry.module.unstable_serializeError) context.errors = sanitizeErrors(context.errors, serverMode);
		}
...
function generateSingleFetchResponse(request, build, serverMode, { result, headers, status }) {
	...
	return new Response(encodeViaTurboStream(result, request.signal, build.entry.module.streamTimeout, serverMode, build.entry.module.unstable_serializeError), {
		status: status || 200,
		headers: resultHeaders
	});
}
...
function encodeViaTurboStream(data, requestSignal, streamTimeout, serverMode, serializeError) {
	...
	return encode(data, {
		...
		plugins: [(value) => {
			if (serializeError && value instanceof Error) {
				...
				if (serialized !== void 0) return ["CustomError", serialized];
			}
		}, (value) => {
